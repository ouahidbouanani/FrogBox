    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="mb-3">
        ...
        <input
          type="text"
          class="form-control"
          v-model="form.idNozzle"
          placeholder="Enter ID Nozzle"
          required
        />
      </div>
...
// Sauvegarde automatique à chaque changement du formulaire
watch(form, (newVal) => {
  localStorage.setItem('draftForm', JSON.stringify(newVal));
}, { deep: true });

// Chargement du brouillon au montage
onMounted(() => {
  const savedForm = localStorage.getItem('draftForm');
  if (savedForm) {
...
const submitForm = async () => {
  try {
    ...
    localStorage.removeItem('draftForm'); // Nettoyage du brouillon
  } catch (error) {
    console.error('Erreur lors de l’envoi du formulaire', error);
  }
};

// Sauvegarde manuelle via bouton "Enregistrer comme brouillon"
const saveDraft = () => {
