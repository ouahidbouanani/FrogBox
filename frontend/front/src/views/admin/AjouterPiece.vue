    ...
    <div class="container w-50 bg-white p-4 rounded shadow  mt-3">
        ...
        <form @submit.prevent="ajouterPiece">
            <div class="mb-3">
                ...
                <input type="text" v-model="nom_piece" class="form-control" required />
            </div>

            <div class="mb-3">
                <label class="form-label">Nombre de côtés</label>
                <input type="number" v-model.number="nb_cotes" class="form-control" min="1" @change="initialiserCotes"
                    required />
            </div>
            ...
            <div v-for="(cote, index) in cotes" :key="index" class="border p-3 mb-3 rounded bg-light">
                <h5>Côté {{ index + 1 }}</h5>
                <div class="mb-2">
                    <label class="form-label">Nom du côté</label>
